...
constexpr int MAX_DECIMAL_PLACES = 10; // used in movavg to avoid precision issues
...
DataFrame movavg (DataFrame df, vector<int> &list_cols_types, 
  ...
  for (int col_idx = 1; col_idx < df.size(); ++col_idx) {
    ...
    // Round to avoid lost of precision due to numerical operations on float
    df_ret[col_idx] = Rcpp::round (col_values_ret, MAX_DECIMAL_PLACES);
