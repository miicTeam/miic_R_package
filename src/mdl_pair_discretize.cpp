...
#define MIN_BIN_SIZE fmin(50, n/10)
#define COEFF_COMB 0
...
double* optfun_onerun_kmdl_coarse(int *sortidx_var, int *data, int nbrV, int **factors, int *r, double pxy,
								  ...
								  double** sc_look, int cplx, int flag_onebin=1) {
	...
	double* Hk_kj=(double *)calloc(nbrV,sizeof(double)); //herve
	...
	while(ir<coarse){
		...
		for(m=0;m<nbrV;m++){
			...
			if(nxyu[m][xyu] != 1){
				//H_0k[m][0] -= nxyu[m][xyu]*looklog[nxyu[m][xyu]] - (nxyu[m][xyu]-1)*looklog[nxyu[m][xyu]-1];
				Hk_0k[m][0] += weights[m]*lookH[nxyu[m][xyu]-1]; //herve
				H_0k[m][0] += weights[m]*lookH[nxyu[m][xyu]-1]; //herve
			}
			if(nbrV==3){
				if(m == 0){
					if(cplx==0 && nxyu[m][xyu]==1) Hk_0k[m][0]  -= 2 * pxy * sc * looklog[n];  //MDL
					else if(cplx==1) Hk_0k[m][0] -= pxy * ( computeLogC(nxyu[m][xyu], sc_levels1, sc_look) - computeLogC(nxyu[m][xyu]-1, sc_levels1, sc_look) ) +
													pxy * ( computeLogC(nxyu[m][xyu], sc_levels2, sc_look) - computeLogC(nxyu[m][xyu]-1, sc_levels2, sc_look) );//NML;//NML
				}
			}
			else{
				if(m == 1){ //herve
				if(cplx==0 && nxyu[m][xyu]==1) Hk_0k[m][0]  -= pxy * sc * looklog[n];  //MDL
				...
				else if(cplx==1) Hk_0k[m][0] -= pxy * ( computeLogC(nxyu[m][xyu], sc_levels1, sc_look) - computeLogC(nxyu[m][xyu]-1, sc_levels1, sc_look) );//NML
				}else if(m == 2){ //herve
				if(cplx==0 && nxyu[m][xyu]==1) Hk_0k[m][0]  -= pxy * sc * looklog[n];  //MDL
				else if(cplx==1) Hk_0k[m][0] -= pxy * ( computeLogC(nxyu[m][xyu], sc_levels2, sc_look) - computeLogC(nxyu[m][xyu]-1, sc_levels2, sc_look) );//NML
				}else if(m == 3){ //herve
				if(cplx==0 && nxyu[m][xyu]==1) Hk_0k[m][0]  -= (1-pxy) * sc * looklog[n];  //MDL
				else if(cplx==1) Hk_0k[m][0] -= (1-pxy) * ( computeLogC(nxyu[m][xyu], sc_levels1, sc_look) - computeLogC(nxyu[m][xyu]-1, sc_levels1, sc_look) +
															computeLogC(nxyu[m][xyu], sc_levels2, sc_look) - computeLogC(nxyu[m][xyu]-1, sc_levels2, sc_look) );//NML
				}
			}
		}
	...
	for(j=1;j<=np-1;j++){ //j=1...n-1
		...
		while((ir<coarse )&& (njforward<n)){
			...
			for(m=0;m<nbrV;m++){
				...
				if(nxyu[m][xyu] != 1){
				Hk_0k[m][j] += weights[m]*lookH[nxyu[m][xyu]-1]; //herve
				H_0k[m][j] += weights[m]*lookH[nxyu[m][xyu]-1]; //herve
				}

				if(nbrV==3){
					if(m == 0){
						if(cplx==0 && nxyu[m][xyu]==1) Hk_0k[m][j] -= 2 * pxy * sc * looklog[n];  //MDL
						else if(cplx==1) Hk_0k[m][j] -= pxy * ( computeLogC(nxyu[m][xyu], sc_levels1, sc_look) - computeLogC(nxyu[m][xyu]-1, sc_levels1, sc_look) ) +
														pxy * ( computeLogC(nxyu[m][xyu], sc_levels2, sc_look) - computeLogC(nxyu[m][xyu]-1, sc_levels2, sc_look) );//NML;//NML
					}
				}
				else{
					if(m == 1){ //herve
					if(cplx==0 && nxyu[m][xyu]==1) Hk_0k[m][j]  -= pxy * sc * looklog[n];  //MDL
					//else if(cplx==1) H_0k[m][j] -= pxy * ( computeLogC(nxyu[m][xyu], fmin(sc_levels1, ALPHA_EFF_LVLS*rx_kj_diff[0][njforward+1]), sc_look) +
								//						   	logchoose(sc_levels1, fmin(sc_levels1, ALPHA_EFF_LVLS*rx_kj_diff[0][njforward+1]), looklog) -
								//				   		   computeLogC(nxyu[m][xyu]-1, fmin(sc_levels1, ALPHA_EFF_LVLS*rx_kj_diff[0][njforward]), sc_look) -
								// 							   	logchoose(sc_levels1, fmin(sc_levels1, ALPHA_EFF_LVLS*rx_kj_diff[0][njforward]), looklog) );//NML
					else if(cplx==1) Hk_0k[m][j] -= pxy * ( computeLogC(nxyu[m][xyu], sc_levels1, sc_look) - computeLogC(nxyu[m][xyu]-1, sc_levels1, sc_look) );//NML
					}else if(m == 2){ //herve
					if(cplx==0 && nxyu[m][xyu]==1) Hk_0k[m][j]  -= pxy * sc * looklog[n];  //MDL
					else if(cplx==1) Hk_0k[m][j] -= pxy * ( computeLogC(nxyu[m][xyu], sc_levels2, sc_look) - computeLogC(nxyu[m][xyu]-1, sc_levels2, sc_look) );//NML
					}else if(m == 3){ //herve
					if(cplx==0 && nxyu[m][xyu]==1) Hk_0k[m][j]  -= (1-pxy) * sc * looklog[n];  //MDL
					else if(cplx==1) Hk_0k[m][j] -= (1-pxy) * ( computeLogC(nxyu[m][xyu], sc_levels1, sc_look) - computeLogC(nxyu[m][xyu]-1, sc_levels1, sc_look) +
																computeLogC(nxyu[m][xyu], sc_levels2, sc_look) - computeLogC(nxyu[m][xyu]-1, sc_levels2, sc_look) );//NML
					}
				}
		...
		Ik[j] = Ik_0k[j];
		I[j] = I_0k[j];
		if((j==(np-1)) && (flag_onebin==1) ){
			//Ik[j] = -DBL_MAX; // Force at least 2 bins
		}


		Imax=-DBL_MAX;

		for(m=0;m<nbrV;m++) Hk_kj[m]=Hk_0k[m][j]; //herve
		...
		for(k=0;k<=j-1;k++){//k=1...n-2 possible cuts
					...
					while( ir<coarse ){
						...
						Hk_kj[m] -= weights[m]*lookH[nxyu_k[m][xyu]]; //herve
						H_kj[m] -= weights[m]*lookH[nxyu_k[m][xyu]]; //herve

						if(nbrV==3){
							if(m == 0){
								if(cplx==0 && nxyu_k[m][xyu]==1) Hk_kj[m]  += 2 * pxy * sc * looklog[n];  //MDL
								else if(cplx==1) Hk_kj[m] -= pxy * ( computeLogC(nxyu_k[m][xyu], sc_levels1, sc_look) - computeLogC(nxyu_k[m][xyu]+1, sc_levels1, sc_look) ) +
															 pxy * ( computeLogC(nxyu_k[m][xyu], sc_levels2, sc_look) - computeLogC(nxyu_k[m][xyu]+1, sc_levels2, sc_look) );//NML;//NML
							}
						}
						else{
							if(m == 1){ //herve
								if(cplx==0 && nxyu_k[m][xyu]==0) Hk_kj[m]  += pxy * sc * looklog[n];  //MDL
			...
			//                                    				  computeLogC(nxyu_k[m][xyu]+1, fmin(sc_levels1, ALPHA_EFF_LVLS*rx_kj_diff[nkforward][njforward-1]), sc_look) -
								...
								else if(cplx==1) Hk_kj[m] -= pxy * ( computeLogC(nxyu_k[m][xyu], sc_levels1, sc_look) - computeLogC(nxyu_k[m][xyu]+1, sc_levels1, sc_look) );//NML
							}else if(m == 2){ //herve
								if(cplx==0 && nxyu_k[m][xyu]==0) Hk_kj[m]  += pxy * sc * looklog[n];  //MDL
								else if(cplx==1) Hk_kj[m] -= pxy * ( computeLogC(nxyu_k[m][xyu], sc_levels2, sc_look) - computeLogC(nxyu_k[m][xyu]+1, sc_levels2, sc_look) );//NML
							}else if(m == 3){ //herve
								if(cplx==0 && nxyu_k[m][xyu]==0) Hk_kj[m]  += (1-pxy) * sc * looklog[n];  //MDL
								else if(cplx==1) Hk_kj[m] -= (1-pxy) * ( computeLogC(nxyu_k[m][xyu], sc_levels1, sc_look) - computeLogC(nxyu_k[m][xyu]+1, sc_levels1, sc_look) +
																		computeLogC(nxyu_k[m][xyu], sc_levels2, sc_look) - computeLogC(nxyu_k[m][xyu]+1, sc_levels2, sc_look) );//NML
							}
						}
					...
					Ik_kj=0;
					I_kj=0;
					for(m=0;m<nbrV;m++)	Ik_kj += Hk_kj[m]; //herve
					for(m=0;m<nbrV;m++)	I_kj += H_kj[m]; //herve
						...
						printf("  ");
						for(m=0;m<nbrV;m++) printf("H_kj[%d]=%lf ",m,H_kj[m]);fflush(stdout);
						printf("\n   [%d -%d][%d - %d] = %.2f (%.2f+%.2f = %.2f)\n",0,k,k+1,j, I[j], I[k], I_kj, I[k]+I_kj);

					#endif

					nk=nkforward-1;//position of the actual possible cut

					if( (Ik[k] + Ik_kj > Ik[j]) && (nkj > MIN_BIN_SIZE) ){ //herve
	...
	//
		...
		for(stop=1;stop<STEPMAX;stop++)
		{
			...
			if(ptr_cnt[ptrVarIdx[0]]==1){
				...
				MInew=optfun_onerun_kmdl_coarse(sortidx[ptrVarIdx[0]], data[ptrVarIdx[0]], 2, factors1, rt1, pxy,
...
int** compute_Ixy_cond_u_new_new_alg1(int** data, int** sortidx, int* ptr_cnt, int* ptrVarIdx,  int* AllLevels, int nbrUi,
								      int n, int maxbins, int **cut, int *r, double* c2terms, int init_nbin, int lbin,
								      double* looklog, double** looklbc, double* lookH, double** sc_look, int cplx, double pxy)
{
	...
	int **factors2=(int **)calloc(2,sizeof(int*));
	int *rt2=(int *)calloc(2,sizeof(int));
	...
	for(stop=1;stop<STEPMAX;stop++)
	{
		...
		for(l=0;l<nbrUi;l++){
			...
			if(ptr_cnt[ptrVarIdx[l+2]]==1){
				...
				//init variables for the optimization run
				factors3[0]=singlefactor;//Ui
				factors3[1]=datafactors[0];//x
				factors3[2]=datafactors[1];//y
				...
				//optimization run on ptrVarIdx[l+2]
				MInew=optfun_onerun_kmdl_coarse(sortidx[ptrVarIdx[l+2]], data[ptrVarIdx[l+2]], 3, factors3, rt3, 1,
												sc, sc_levels1, sc_levels2, n, AllLevels[ptrVarIdx[l+2]], cut[l+2],
												&(r[l+2]), maxbins, looklog, looklbc, lookH, sc_look, cplx); // 3 factors
		...
		///////////////////////////////////////////
		//optimize I(x;yu) over x
		if(ptr_cnt[ptrVarIdx[0]]==1){
			jointfactors_uiyx(datafactors, -1,  n, nbrUi, r, uiyxfactors, ruiyx);

			//init variables for the optimization run
			factors2[0]=uiyxfactors[1];//yu
			factors2[1]=singlefactor;//x
			...
			sc_levels2 = r[0];
			// Run optimization on X.
			MInew=optfun_onerun_kmdl_coarse(sortidx[ptrVarIdx[0]], data[ptrVarIdx[0]], 2, factors2, rt2, 1,
											sc, sc_levels1, sc_levels2, n, AllLevels[ptrVarIdx[0]], cut[0],
											&(r[0]), maxbins, looklog, looklbc, lookH, sc_look, cplx); // 2 factors

			update_datafactors(sortidx, ptrVarIdx[0], datafactors, 0, n, cut);
		}

		///////////////////////////////////////////
		//optimize I(y;xu) over y
		if(ptr_cnt[ptrVarIdx[1]]==1){
			jointfactors_uiyx(datafactors, -1,  n, nbrUi, r, uiyxfactors, ruiyx);

			//init variables for the optimization run
			factors2[0]=uiyxfactors[2];//xu
			factors2[1]=singlefactor;//y
			...
			sc_levels2 = r[1];
			// Run optimization on Y.
			MInew=optfun_onerun_kmdl_coarse(sortidx[ptrVarIdx[1]], data[ptrVarIdx[1]], 2, factors2, rt2, 1,
											sc, sc_levels1, sc_levels2, n, AllLevels[ptrVarIdx[1]], cut[1],
											&(r[1]), maxbins, looklog, looklbc, lookH, sc_look, cplx); // 2 factors
		...
		if (cplx == 0){ //MDL //cplx
			res = computeMIcond_kmdl(uiyxfactors, ruiyx, r, n, looklog);
		}else if(cplx == 1){ //NML
	...
	for(l=0; l<(nbrUi+2); l++){
		iterative_cuts[stop][l*maxbins]=-1; // mark where we stopped iterating
		iterative_cuts[stop][l*maxbins+1]=10000*res[1]; // pass Ik[X;Y|U]
		iterative_cuts[stop][l*maxbins+2]=10000*res[0]; // pass I[X;Y|U]
	}
								  ...
								  double* looklog, double** looklbc, double* lookH, double** sc_look, int cplx, double pxy)
{
	...
	for(stop1=1;stop1<STEPMAX1;stop1++)
	{
		...
		for(stop=1;stop<2;stop++) //optimize I(y;xu) over x and u
		{
			///////////////////////////////////////////
				...
				for(l=0;l<nbrUi;l++){
					...
					if(ptr_cnt[ptrVarIdx[l+2]]==1){
						...
						MInew=optfun_onerun_kmdl_coarse(sortidx[ptrVarIdx[l+2]], data[ptrVarIdx[l+2]], 2, factors1, rt1, 1,
														sc, sc_levels1, sc_levels2, n, AllLevels[ptrVarIdx[l+2]], cut[l+2],
														&(r[l+2]), maxbins, looklog, looklbc, lookH, sc_look, cplx); // 2 factors
			...
			if(ptr_cnt[ptrVarIdx[0]]==1){
				...
				sc_levels2 = sc_levels_x; //herve
				// Run optimization on X.
				res=optfun_onerun_kmdl_coarse(sortidx[ptrVarIdx[0]], data[ptrVarIdx[0]],2, factors1, rt1,
												1, sc, sc_levels1, sc_levels2, n, AllLevels[ptrVarIdx[0]],
												cut[0], &(r[0]), maxbins, looklog, looklbc, lookH, sc_look, cplx); // 2 factors
				//for(ll=0;ll<nbrUi+2;ll++) printf("r[%d]=%d ",ll,r[ll]);
				//printf("\n\n");
				update_datafactors(sortidx, ptrVarIdx[0], datafactors, 0, n, cut);
			}
			...
			if(flag) break;
			MIk[stop]=res[1];
		}// opt loop on I(y;xu) over x and u.
		...
		r_temp[2]=ruiyx[3];
		//if(cplx == 1)
		//	res=computeMI_knml(datafactors[1],uiyxfactors[2], uiyxfactors[3],r_temp,n,c2terms, looklog);
		//else
		//	res=computeMI_kmdl(datafactors[1],uiyxfactors[2], uiyxfactors[3],r_temp,n, looklog);
		I_y_xu = res[0]; //After optimization on X and U.
		Ik_y_xu = res[1];
		// Reset cutpoints on U
		reset_u_cutpoints(cut, nbrUi, ptr_cnt, ptrVarIdx, init_nbin, maxbins, lbin, r, AllLevels, n);
		for(l=0; l<nbrUi; l++) update_datafactors(sortidx, ptrVarIdx[l+2], datafactors, l+2, n, cut);
        // Reset MIk and MI
		for(i=0;i<STEPMAX;i++){
	        MIk[i]=0;
	        MI[i]=0;
		...
		for(stop=1; stop<STEPMAX; stop++) //optimize I(x;u) over u
		{
			///////////////////////////////////////////
		...
		r_temp[2]=ruiyx[2];
		//if(cplx == 1)
		//	res=computeMI_knml(datafactors[0],uiyxfactors[0], uiyxfactors[2],r_temp,n,c2terms, looklog);
		//else
		//	res=computeMI_kmdl(datafactors[0],uiyxfactors[0], uiyxfactors[2],r_temp,n, looklog);
		...
		for(stop=1;stop<2;stop++) //optimize I(x;yu) over y and u
		{
			///////////////////////////////////////////
			...
			if(ptr_cnt[ptrVarIdx[0]]==1){
				...
				sc_levels2 = sc_levels_y; //herve
				// Run optimization on Y.
				res=optfun_onerun_kmdl_coarse(sortidx[ptrVarIdx[1]], data[ptrVarIdx[1]],2, factors1, rt1,
												1, sc, sc_levels1, sc_levels2, n, AllLevels[ptrVarIdx[1]],
												cut[1], &(r[1]), maxbins, looklog, looklbc, lookH, sc_look, cplx); // 2 factors

				update_datafactors(sortidx, ptrVarIdx[1], datafactors, 1, n, cut);
			}

			//res[1]=MInew[1]; //Ik(x;yu) ?
			//////////////////////////////////////////
			for(i=stop-1;i>0;i--){
				if( (fabs(res[1]-MIk[i]) < EPS) ){ // If no real improvement over last information
					flag=1;
			...
			if(flag) break;
			MIk[stop]=res[1];
		}// opt loop on I(x;yu) over y and u.
		...
		I_x_yu = res[0]; //After optimization on Y and U.
		Ik_x_yu = res[1];
		// Reset cutpoints on U
		reset_u_cutpoints(cut, nbrUi, ptr_cnt, ptrVarIdx, init_nbin, maxbins, lbin, r, AllLevels, n);
		for(l=0; l<nbrUi; l++) update_datafactors(sortidx, ptrVarIdx[l+2], datafactors, l+2, n, cut);
		...
		for(stop=1; stop<STEPMAX; stop++) //optimize I(y;u) over u
		{
			///////////////////////////////////////////
		...
		r_temp[2]=ruiyx[1];
		//if(cplx == 1)
		//	res=computeMI_knml(datafactors[1],uiyxfactors[0], uiyxfactors[1],r_temp,n,c2terms, looklog);
		//else
...
// NEW COND ALG
int** compute_Ixy_cond_u_new_2_alg1(int** data, int** sortidx, int* ptr_cnt, int* ptrVarIdx,  int* AllLevels, int nbrUi,
								  int n, int maxbins, int **cut, int *r, double* c2terms, int init_nbin, int lbin,
								  double* looklog, double** looklbc, double* lookH, double** sc_look, int cplx, double pxy)
{
	...
	for(stop=1;stop<STEPMAX;stop++) //optimize I(y;xu) over x, y and u
	{
		///////////////////////////////////////////
		//optimize I(y;xu) over u
		counter = 1;
		while(counter>0){
			for(l=0;l<nbrUi;l++){
		...
		if(ptr_cnt[ptrVarIdx[1]]==1){
			...
			//init variables for the optimization run
			factors1[0]=uiyxfactors[2];//ux
			factors1[1]=singlefactor;//y : one bin initially
			...
			sc_levels1 = ruiyx[2];
			sc_levels2 = sc_levels_x; //not used
			// Run optimization on X.
			res=optfun_onerun_kmdl_coarse(sortidx[ptrVarIdx[1]], data[ptrVarIdx[1]],2, factors1, rt1,
											1, sc, sc_levels1, sc_levels2, n, AllLevels[ptrVarIdx[1]],
											cut[1], &(r[1]), maxbins, looklog, looklbc, lookH, sc_look, cplx); // 2 factors
		...
		if(flag) break;
		MIk[stop]=res[1];
	}// opt loop on I(y;xu) over x, y and u.

	// save first cutpoints 
	for(j=0; j<maxbins; j++){
		for(l=0; l<(nbrUi+2); l++){
			iterative_cuts[0][j+l*maxbins] = cut[l][j];
		}
	}
	...
	Ik_y_xu = res[1];
	// Reset cutpoints on U
	reset_cutpoints(cut, nbrUi, ptr_cnt, ptrVarIdx, init_nbin, maxbins, lbin, r, AllLevels, n);
	for(l=0; l<nbrUi+2; l++) update_datafactors(sortidx, ptrVarIdx[l], datafactors, l, n, cut);
	// Reset MIk and MI
	for(i=0;i<STEPMAX;i++){
		MIk[i]=0;
		MI[i]=0;
	...
	for(stop=1;stop<STEPMAX;stop++) //optimize I(x;yu) over x, y and u
	{
		///////////////////////////////////////////
		//optimize I(x;yu) over u
		counter=1;
		while(counter>0) {
			for(l=0;l<nbrUi;l++){
		...
		if(ptr_cnt[ptrVarIdx[0]]==1){
			...
			//init variables for the optimization run
			factors1[0]=uiyxfactors[1];//uy
			factors1[1]=singlefactor;//x: initially 1 bin
			...
			sc_levels1 = ruiyx[1];
			sc_levels2 = sc_levels_y; //not used
			// Run optimization on Y.
			res=optfun_onerun_kmdl_coarse(sortidx[ptrVarIdx[0]], data[ptrVarIdx[0]], 2, factors1, rt1,
											1, sc, sc_levels1, sc_levels2, n, AllLevels[ptrVarIdx[0]],
											cut[0], &(r[0]), maxbins, looklog, looklbc, lookH, sc_look, cplx); // 2 factors
		...
		if(flag) break;
		MIk[stop]=res[1];
	}// opt loop on I(x;yu) over x, y and u.

	// save second cutpoints 
	for(j=0; j<maxbins; j++){
		for(l=0; l<(nbrUi+2); l++){
			iterative_cuts[1][j+l*maxbins] = cut[l][j];
		}
	}
	...
	Ik_x_yu = res[1];
	// Reset cutpoints on U
	reset_cutpoints(cut, nbrUi, ptr_cnt, ptrVarIdx, init_nbin, maxbins, lbin, r, AllLevels, n); // and X, Y?
	for(l=0; l<nbrUi+2; l++) update_datafactors(sortidx, ptrVarIdx[l], datafactors, l, n, cut);
	// Reset MIk and MI
	for(i=0;i<STEPMAX;i++){
		MIk[i]=0;
	...
	for(stop=1; stop<STEPMAX; stop++) //optimize I(x;u) over x and u
	{
		///////////////////////////////////////////
		//optimize I(x;u) over u
		for(l=0;l<nbrUi;l++){
		...
		///////////////////////////////////////////
		//optimize I(x;u) over x
		if(ptr_cnt[ptrVarIdx[0]]==1){
			//optimize on x
			//I(x;u)

			//init variables for the optimization run
			factors1[0]=uiyxfactors[0];//u
			factors1[1]=singlefactor;//x: initially 1 bin
	...
	for(stop=1; stop<STEPMAX; stop++) //optimize I(y;u) over y and u
	{
		///////////////////////////////////////////
		//optimize I(y;u) over u
		for(l=0;l<nbrUi;l++){
		...
		if(ptr_cnt[ptrVarIdx[1]]==1){
			...
			//init variables for the optimization run
			factors1[0]=uiyxfactors[0];//u
			factors1[1]=singlefactor;//y: initially 1 bin
	...
	if(cplx == 1)
		...
		res=computeMI_kmdl(datafactors[1],uiyxfactors[0], uiyxfactors[1],r_temp,n, looklog, 1);
	I_y_u = res[0]; //After optimization on U.
	Ik_y_u = res[1];
	...
	for(l=0; l<(nbrUi+2); l++){
		iterative_cuts[3][l*maxbins]=-1; // mark where we stopped iterating
		iterative_cuts[3][l*maxbins+1]=10000*res[1]; // pass Ik[X;Y|U]
		iterative_cuts[3][l*maxbins+2]=10000*res[0]; // pass I[X;Y|U]
	}
								...
								double* looklog, double** looklbc, double* lookH, double** sc_look, int cplx, double pxy)
{
	...
	for(stop1=1;stop1<STEPMAX1;stop1++)
	{
		...
		flag=0;
		for(stop=1;stop<2;stop++) //optimize I(y;xu) over x and I(x;yu) over y
		{
			///////////////////////////////////////////
			...
			if(ptr_cnt[ptrVarIdx[0]]==1){
				...
				//else sc_levels_y = 0.5*(rt1[0]-1);
				else sc_levels_y = (sc_levels_y*(stop1-1) + r[1])/stop1; //Harmonic mean of previous levels.
				sc_levels_y = r[1];
				sc = 0.5*(sc_levels_y-1);
			...
			if(ptr_cnt[ptrVarIdx[1]]==1){
				...
				factors1[0]=uiyxfactors[2];//xu
				factors1[1]=uiyxfactors[0];//u
				//factors1[1]=singlefactor;//u //herve
				...
				//rt1[1]=1; // one single bin for y initially //herve
				if (stop1 == 1){
					sc_levels_x = r[0];//rt1[0];
				}
				else sc_levels_x = (sc_levels_x*(stop1-1) + r[0])/stop1; //Harmonic mean of previous levels.
				sc_levels_x = rx;
				sc = 0.5*(sc_levels_x-1);
				...
				// Run optimization on Y.
				//MInew=optfun_onerun_kmdl_coarse(sortidx[ptrVarIdx[1]], data[ptrVarIdx[1]],1, factors1, rt1, optfun1, sc, sc_levels_x, n, AllLevels[ptrVarIdx[1]], cut[1], &(r[1]), maxbins, looklog, looklbc, lookH, sc_look, cplx);
				MInew=optfun_onerun_kmdl_coarse(sortidx[ptrVarIdx[1]], data[ptrVarIdx[1]],2, factors1, rt1,
												1, sc, sc_levels1, sc_levels2, n, AllLevels[ptrVarIdx[1]],
												cut[1], &(r[1]), maxbins, looklog, looklbc, lookH, sc_look, cplx); // 2 factors //herve
	    ...
	    for(stop=1;stop<2;stop++)
		{
		  ...
		  for(l=0;l<nbrUi;l++){
			...
			factors3[0]=uiyxfactors[3];//xyu
			factors3[1]=uiyxfactors[1];//yu
			factors3[2]=uiyxfactors[2];//xu
			factors3[3]=uiyxfactors[0];//u
			...
			//pxy = pxy; //herve can be varied from 0 to 1
			sc_levels1 = r[0];//sc_levels_x; //herve
			sc_levels2 = r[1];//sc_levels_y; //herve
	...
	if(nbrUi==0){
		...
		for(l=0;l<2;l++){
			if(ptr_cnt[ptrVarIdx[l]]==1){
				...
				fflush(stdout);
		#endif
		int **iterative_cuts = compute_Ixy_alg1(data, sortidx, ptr_cnt, ptrVarIdx, AllLevels, n,
...
extern "C" SEXP mydiscretizeMutual(SEXP RmyDist1, SEXP RmyDist2, SEXP RflatU, SEXP RnbrU, SEXP RmaxBins,
  ...
  ncol = 10000; // combinations of Us can exceed ncol
