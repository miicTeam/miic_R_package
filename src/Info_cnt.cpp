    ...
    for(uint j=0;j<np;j++){ //j=1...n-1
        ...
        for(int level=0; level<r[1]; level++){
            ...
            if(environment.cplx==0 && counts[1][level]>0) Hk_kj[1] -= sc * environment.looklog[n];
                ...
                Hk_kj[1] -= computeLogC(weighted_count, sc_levels1, environment.looklog, environment.cterms);
            }
        }
        ...
        for(uint k=0;k<j;k++){//k=1...n-2 possible cuts
            ...
            if((Ik[k] + Ik_kj) > Ik[j]) {
                I_newbin = Ik[k] + Ik_kj; //[0.. cuts.. k-1][k j] //herve
                if (Imax < I_newbin){
                    Imax = I_newbin;
                    ...
                    memory_cuts_pos[j] = nkforward-1; // position  of the (last) optimal cut
                }
            }
        }
                         ...
                         int n, int **cut, int *r, Environment& environment, bool saveIterations)
{
    ...
    for(int new_initbins=2; (new_initbins<initbins) && (new_initbins<20) && (new_initbins<min_unique_values); new_initbins++){
    ...
    if(saveIterations){
        environment.iterative_cuts[stop][0]=-1; // mark where we stopped iterating
        environment.iterative_cuts[stop][1]=100000*return_res[1]; // Pass Ik[X;Y]
        environment.iterative_cuts[stop][2]=100000*return_res[0]; // Pass I[X;Y]
        environment.iterative_cuts[stop][3]=100000*max_res; // Pass max res before optimization with equal freq
                                    ...
                                    bool saveIterations)
{
    ...
    for(stop1=1;stop1<STEPMAX1;stop1++)
    {
        ...
        //    if((ptr_cnt[ptrVarIdx[ll+2]]==1) && (r_old[ll+2]>1) && (r_old[ll+2] < np))
        //        Ik_y_xu -= logchoose(np-1, r_old[ll+2]-1, looklog, lookchoose)/n;
        ...
        //    np = min(AllLevels[ptrVarIdx[ll+2]], maxbins);
        //    if((ptr_cnt[ptrVarIdx[ll+2]]==1) && (r_old[ll+2]>1) && (r_old[ll+2] > np))
        //        Ik_x_yu -= logchoose(np-1, r_old[ll+2]-1, looklog, lookchoose)/n;
        ...
        //    np = min(AllLevels[ptrVarIdx[ll+2]], maxbins);
        //    if((ptr_cnt[ptrVarIdx[ll+2]]==1) && (r_old[ll+2]>1) && (r_old[ll+2] > np))
        //        Ik_x_u -= logchoose(np-1, r_old[ll+2]-1, looklog, lookchoose)/n;
        ...
        //    np = min(AllLevels[ptrVarIdx[ll+2]], maxbins);
        //    if((ptr_cnt[ptrVarIdx[ll+2]]==1) && (r_old[ll+2]>1) && (r_old[ll+2] > np))
        //        Ik_y_u -= logchoose(np-1, r_old[ll+2]-1, looklog, lookchoose)/n;
    ...
    if(saveIterations){
        for(l=0; l<(nbrUi+2); l++){
            environment.iterative_cuts[stop1][l*maxbins]=-1; // mark where we stopped iterating
            environment.iterative_cuts[stop1][l*maxbins+1]=100000*return_res[1]; // pass Ik[X;Y|U]
            environment.iterative_cuts[stop1][l*maxbins+2]=100000*return_res[0]; // pass I[X;Y|U]
        }
    }
                             ...
                             int n, Environment& environment, bool saveIterations)
{
    ...
    if(nbrUi==0){
        ...
        res_temp=compute_Ixy_cond_u_new_alg1(data, sortidx, ptr_cnt, ptrVarIdx, AllLevels, nbrUi, n,
                                             cut, r, lbin, environment, saveIterations);
