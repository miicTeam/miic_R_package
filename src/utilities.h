...
std::vector<std::vector<std::string> > saveEdgesListAsTable(structure::Environment&);
...
std::string vectorToStringNodeName(const structure::Environment&, const std::vector<int>&);
void readTime(structure::Environment&, std::string);
void readFilesAndFillStructures(std::vector<std::string> edgesVectorOneLine,
		structure::Environment&);
bool readBlackbox(std::vector<std::string>, structure::Environment&);
std::vector<std::vector<std::string> > getAdjMatrix(const structure::Environment&);
int sign(double val);
...
double** getJointFreqs(structure::Environment&, int i, int j, int numSamples_nonNA);
