...
# - all possible parameters specific to temporal mode
...
tmiic_check_parameters <- function (list_in,
  ...
  if ( ! is.null (n_layers) )
    ...
    if ( "n_layers" %in% colnames(list_in$non_lagged$state_order) )
      ...
      {
      # The n_layers parameter is used, so we need to check it
      # and it must be an int >= 2
      #
      if ( test_param_wrong_int (n_layers, min=2) )
        miic_warning ("parameters", "supplied value ", list_to_str (n_layers),
  ...
  if ( ! is.null (mov_avg) )
    ...
    if ( "mov_avg" %in% colnames(list_in$non_lagged$state_order) )
      ...
      if (   test_param_wrong_int (mov_avg, min=0)
        ...
        #
        # No mov_avg on contextual vars (constant per trajectory)
        #
...
tmiic_check_state_order <- function (list_in)
  ...
  if (  ( ! ("n_layers" %in% colnames(list_in$non_lagged$state_order)) )
    ...
    if ( any (na_in_so) )
      ...
      # - if there are multiple n_layers values, can't decide, stop
      #
    ...
    #
    # Extra check for contextual vars, n_layers must be 1
    #
  ...
  if (  ( ! ("delta_t" %in% colnames(list_in$non_lagged$state_order)) )
    ...
    if ( any (na_in_so) )
      ...
      # - if there are multiple delta_t values, stop
      #
  ...
  if (  ( ! ("mov_avg" %in% colnames(list_in$non_lagged$state_order)) )
    ...
    #
    # For contextual vars, no moving average
    #
...
tmiic_prepare_inputs <- function (list_in,
  ...
  #
  list_ret$state_order <- tmiic_lag_state_order (list_ret$non_lagged$state_order)
  list_ret$true_edges <- tmiic_lag_bb_te (list_ret$non_lagged$state_order,
                                          list_ret$non_lagged$true_edges)
  list_ret$black_box <- tmiic_lag_bb_te (list_ret$non_lagged$state_order,
                                         list_ret$non_lagged$black_box)
  list_traj_lagged <- tmiic_lag_input_data (list_traj,
