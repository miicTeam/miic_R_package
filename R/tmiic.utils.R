...
#
# Description: Utility functions for temporal modes of MIIC
#
# Author     : Franck SIMON
#
# Changes history:
...
# Returns: none
#-------------------------------------------------------------------------------
tmiic_check_parameters_not_temporal <- function (n_layers, delta_t, movavg,
  ...
  if ( ! is.null (delta_t) )
    miic_warning ("parameters", "the delta_t parameter ", delta_t,
      " will not be used as mode is not temporal.")
  if ( ! is.null (movavg) )
    miic_warning ("parameters", "the movavg parameter ", movavg,
      " will not be used as mode is not temporal.")
...
#-------------------------------------------------------------------------------
# Checks parameters in temporal mode
#
...
# per variable: n_layers, delta_t and movavg, if these variables are supplied
...
# The move into the state_order is tuned for each variable, i.e.: moavg is
...
# - non_lagged$state_order: n_layers, delta_t, movavg can be added as new
#   columns in the non lagged state_order:
# - params: the list of parameters will include checked temporal parameters
#-------------------------------------------------------------------------------
tmiic_check_parameters <- function (list_in, n_layers, delta_t, movavg,
  keep_max_data, max_nodes, var_interest, var_interest_condition, window_position)
  {
  ...
  if ( ! is.null (movavg) )
    {
    if ( "movavg" %in% colnames(list_in$non_lagged$state_order) )
      miic_warning ("parameters", "supplied value ", list_to_str (movavg),
        " for the movavg parameter will be ignored as the moving average",
        " is also provided in the state_order.")
    else
      {
      if (   test_param_wrong_int (movavg, min=0, max=NA)
        ...
        # Valid movavg
        #
        list_in$non_lagged$state_order$movavg <- movavg
        #
        # No movavg on discrete
        #
        list_in$non_lagged$state_order$movavg[list_in$non_lagged$state_order$var_type == 0] <- 0
        #
        # No movavg on contextual vars (constant in stationary
        ...
        # the state_order
        #
        list_in$non_lagged$state_order$movavg[list_in$non_lagged$state_order$is_contextual == 1] <- 0
        }
      }
    }
...
# This function check n_layers, delta_t and movavg in term of possible types
...
tmiic_check_state_order <- function (list_in)
  ...
  #
  if (  ( ! ("movavg" %in% colnames(list_in$non_lagged$state_order)) )
     || all(is.na(list_in$non_lagged$state_order$movavg)) )
    {
    # Add movavg column with 0 for all vars
    #
    list_in$non_lagged$state_order$movavg <- 0
    }
  else
    {
    # movavg supplied, check values
    #
    wrongs <- unlist (lapply (list_in$non_lagged$state_order$movavg, FUN=function(x) {
      if  (is.na (x))                   # NA: OK (missing row added before)
        return (FALSE)
      else if ( is.na ( suppressWarnings (as.numeric(x)) ) ) # Not num: KO
        return (TRUE)
    ...
    if ( any (wrongs) )
      ...
      list_in$non_lagged$state_order$movavg[wrongs] <- NA
      }
    list_in$non_lagged$state_order$movavg <- as.integer (list_in$non_lagged$state_order$movavg)
    #
    # Replace NA vals by 0
    #
    na_in_so <- is.na (list_in$non_lagged$state_order$movavg)
    if ( any (na_in_so) )
      {
      msg_str <- list_to_str (list_in$non_lagged$state_order$var_names[na_in_so],
                              n_max=10)
      ...
      list_in$non_lagged$state_order$movavg[na_in_so] <- 0
      }
    #
    # Check/fix invalid values: for discrete vars, no moving average
    #
    wrongs <- ( (list_in$non_lagged$state_order$movavg != 0)
    ...
    if ( any (wrongs) )
      ...
      list_in$non_lagged$state_order$movavg[wrongs] <- 0
      }
    #
    # In stationary mode, for contextual vars, no moving average
    #
    wrongs <- ( (list_in$non_lagged$state_order$movavg != 0) & are_contextual )
    if ( any (wrongs) )
      {
      msg_str <- list_to_str (list_in$non_lagged$state_order$var_names[wrongs],
                              n_max=10)
    ...
    # but don't change the movavg value (it is the user choice)
    #
    if (list_in$params$mode == "TNS")
      {
      wrongs <- ( (list_in$non_lagged$state_order$movavg != 0)
    ...
    #
    uniq_vals <- unique ( list_in$non_lagged$state_order$movavg[
        (list_in$non_lagged$state_order$movavg != 0) ] )
    if (length (uniq_vals) > 1)
      {
      msg_str <- list_to_str (uniq_vals)
...
#   The length of the movavg list is the number of columns of the data frames - 1
...
#-------------------------------------------------------------------------------
tmiic_movavg <- function (list_traj, movavg=NULL, keep_max_data=F, verbose_level=0)
  {
  ...
  for (i in 1:length(list_traj) )
    for (j in 1:n_vars)
      if (movavg[[j]] >= 2)
        ...
        list_traj[[i]][,j+1] <- tmiic_movavg_onecol (list_traj[[i]][,j+1], movavg[[j]])
        if (verbose_level >= 2)
...
tmiic_check_data_after_lagging <- function (lagged_data, lagged_so)
  ...
  #
  # Warning on columns full of NAs + remove rows
  #
...
tmiic_prepare_inputs <- function (list_in,
  ...
  # NB: if n_layers, delta_t and movavg are supplied as parameters, but not
  ...
  list_ret <- tmiic_check_parameters (
    ...
    window_position = window_position)
  #
  # Init / check (/ harmonize if possible) n_layers, delta_t and movavg
  #
  ...
  list_traj <- tmiic_movavg (list_traj, list_ret$non_lagged$state_order$movavg,
