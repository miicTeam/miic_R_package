  ...
  for (one_voi_idx in 1:n_all_vois)
    ...
    while (start_idx <= n_vars)
      ...
      if (!skip_cheks)
        ...
        non_constant_cols <- names(count_vals[count_vals >= 2])
        if (!("var_interest" %in% non_constant_cols)) {
          # keep the variable of interest even if it is constant
          # a warning will be generated by miic at beginning of the run
          non_constant_cols <- c(non_constant_cols, "var_interest")
        }

        data_loop <- data_loop[, non_constant_cols, drop = FALSE]

        count_nas <- apply (data_loop, MARGIN=1, FUN=function(x) { sum (is.na(x) ) } )
        data_loop <- data_loop[ count_nas < ncol(data_loop), , drop=F]

        #if var_interest is not in data_loop after filtering stop 
        if (!("var_interest" %in% colnames(data_loop))) {
          stop(paste0("Variable of interest '", one_voi_name,
            "' is full of NA please check your data , cannot compute MI."))
        }
        }
