...
miic.plot <-function(g, method = "log_confidence", igraphLayout = NULL,
  ...
  if (method == "pcor") {
    if (!is.na(g$retained.edges.summary$partial_correlation[1])) {
      ...
      graphics::plot(c(0, 5), c(-1, 1), type = 'n', axes = F, xlab = '',
                     ylab = '', main = 'Partial correlation', cex.main = 0.7)
      graphics::text(x=1.5, y = seq(-1, 1, l=5), labels = seq(-1, 1, l=5),
                     cex = 0.7)
      graphics::rasterImage(legend_image, 2.5, -1, 3.5, 1)
    } else {
      print(paste("It is not possible to plot correlation for categorical ",
  ...
  # plot the Confidence Graph  only for miic on log_confidence
    ...
    graphics::plot(c(0, 4), c(0.2, 0.8), type = 'n', axes = F, xlab = '',
                   ylab = '')
    graphics::par(adj=1)
