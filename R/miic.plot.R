...
miic.plot <-function(g, method = "log_confidence", igraphLayout = NULL, userLayout = NULL, verbose = F)
{
  ...
  if( verbose == TRUE ) { cat( "# Load the positions of the vertices\n" ) }
  ...
  if( method == "pcor" ){
    if(!is.na(g$retained.edges.summary$partial_correlation[1])){
      ...
      graphics::par(mar=rep(.5, 4), oma=c(3,3,3,1), las=1)
      ...
      graphics::plot(c(0,5),c(-1,1),type = 'n', axes = F,xlab = '', ylab = '', main = 'Partial correlation', cex.main = 0.7)
      graphics::text(x=1.5, y = seq(-1,1,l=5), labels = seq(-1,1,l=5), cex = 0.7)
      graphics::rasterImage(legend_image, 2.5, -1, 3.5,1)
    } else {
      print("It is not possible to plot correlation for categorical variables without an order")
    }
  ...
  else if( method == "log_confidence" )
  {
    ...
    graphics::layout(t(1:3), widths=c(5,1,1))
     ...
     # Set margins and turn all axis labels horizontally (with `las=1`)
    graphics::par(mar=c(.5, .1,.5,.1), oma=c(3,3,3,1), las=1)
    graphics::plot(myGraph, layout = myLayout )
    blue.gradient = grDevices::rainbow(100, start = 3/6, end=4/6)
    red.gradient = grDevices::rainbow(100, start=0, end=0.16)
    # Legend
  ...
  } else { stop("incorrect method argument: must be \"log_confidence\" or \"pcor\" ") }
