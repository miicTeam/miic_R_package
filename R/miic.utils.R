...
check_input_data <- function (input_data, mode)
  ...
  #
  # If mode is not temporal, NAs check about rows with only NAs
  # (in temporal mode, NAs check will be done after data lagging)
  #
...
check_state_order <- function (input_data, state_order, mode)
  ...
  for (i in 1:n_vars)
    ...
    if (state_order[i, "var_type"] == 1) # Continuous
      {
      # Less than 2 unique values does not make sense for a continuous variable
      #
  ...
  if ( ! ("levels_increasing_order" %in% colnames (state_order) ) )
    ...
    for (i in 1:n_vars)
      ...
      if (is.logical (values))
        ...
        orders[!is.na (orders_log)] <- as.character (orders_log[!is.na (orders_log)])
        }
      else if (is.integer (values))
        {
        suppressWarnings ( { orders_int <- as.integer(orders) } )
        orders[!is.na (orders_int)] <- as.character (orders_int[!is.na (orders_int)])
        }
      else if (is.numeric (values))
        {
        suppressWarnings ( { orders_num <- as.numeric(orders) } )
        orders[!is.na (orders_num)] <- as.character (orders_num[!is.na (orders_num)])
        }
...
# - df: the data fame to check, expected to have a specific number of columns
#   All variables names in the data frame are expected to be in the input data.
...
check_other_df <- function (input_data, state_order, df, df_name, mode)
  ...
  if (mode %in% MIIC_TEMPORAL_MODES)
    ...
    if ( any (wrong_lags) )
      {
      if (sum (wrong_lags) == 1)
        miic_warning (df_name, "wrong lag found, this line will be ignored.")
      else
        miic_warning (df_name, "wrong lag for multiple rows, these rows will be ignored.")
...
check_param_string <- function (value, name, possibles)
  ...
  if (  is.null(value)
     ...
     || (!(value %in% possibles)) )
    {
    msg_str <- paste0 (possibles, collapse=", ")
    miic_warning ("parameters", "supplied value ", list_to_str (value),
...
check_parameters <- function (input_data, n_threads, cplx,
  ...
  if ( test_param_wrong_float (ori_proba_ratio, min=0, strict_min=F, max=1) )
    {
    miic_warning ("parameters", "supplied value ", list_to_str (ori_proba_ratio),
      " for the orientation probabilty ratio parameter is invalid.",
      " It must be a floating point in the range ]0,1].",
  ...
  else if ( test_param_wrong_float (ori_consensus_ratio, min=0, strict_min=F, max=1) )
    {
    miic_warning ("parameters", "supplied value ", list_to_str (ori_consensus_ratio),
