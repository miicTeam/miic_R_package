miic.reconstruct <- function(inputData = NULL, cntVar = NULL, blackBox = NULL,
                             stateOrder = NULL, nThreads= nThreads, effN = -1,
                             cplx = c("nml", "mdl"), eta = 1,
                             latent = c("no", "yes", "ort"),
                             confidenceShuffle = 0, edges = NULL,
                             orientation = TRUE, propagation = TRUE,
                             confidenceThreshold = 0, verbose = FALSE,
                             typeOfData = NULL, sampleWeights = NULL,
                             testMAR = TRUE, consistent = c("no",
                                                            "orientation",
                                                            "skeleton")) {
  isTplReuse = TRUE
  isK23 = TRUE
  ...
  if (base::requireNamespace("Rcpp", quietly = TRUE)) {
    res <- .Call('reconstruct', inData, typeOfData, cntVar, numNodes,
                 nThreads, edges, bB, effN, cplx, eta, hvs, latent,
                 isTplReuse, isK23, isDegeneracy, propagation, isNoInitEta,
                 confidenceShuffle, confidenceThreshold, sampleWeights,
                 consistent, testMAR, verbose, PACKAGE = "miic")
    if (res$interrupted)
  ...
  df <- data.frame(matrix(res1, nrow = length(res$edges) - 1, byrow = T),
  ...
  df <- data.frame(matrix(res1, nrow = length(res$adjMatrix)-1,
                          byrow = T),
                   stringsAsFactors = FALSE)
  df <- df[, -1]
  colnames(df) <- tmp
  ...
  if (confidenceShuffle > 0) {
    ...
    df <- data.frame(matrix(res1, nrow = length(confData)-1, byrow = T),
                     stringsAsFactors = FALSE)
    colnames(df) <-tmp
