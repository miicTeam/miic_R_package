miic.reconstruct <- function (list_in)
  ...
  arg_list <- list(
    ...
    "ori_proba_ratio" = list_in$params$ori_proba_ratio,
    ...
    #
    # Parameters part of miic parameters (can be defined by the user)
    #
  ...
  res$adj_matrix <- matrix(unlist(res$adj_matrix),
                           nrow = length(list_in$input_data),
                           byrow = TRUE)
  ...
  if (length(res$proba_adj_matrices) > 0)
    ...
    res$proba_adj_average <- matrix(unlist(adj_average),
                                    nrow = length(list_in$input_data), byrow = TRUE)
    }
  #
  # Execution time
  #
  ...
  if (length(res$orientations.prob) > 0)
    ...
    orientations_prob <- data.frame(
      matrix(res1, nrow = length(orientations_prob) - 1, byrow = TRUE),
