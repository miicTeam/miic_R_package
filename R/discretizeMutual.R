...
discretizeMutual <- function(X, Y, matrixU = NULL, maxbins = NULL,
                             cplx = "nml", Neff = NULL, sample_weights = NULL,
  ...
  if (!(is.vector(X) || is.factor(X)) || !(is.vector(Y) || is.factor(Y)))
    stop(paste("Please provide the two samples X and Y as numerical vectors",
               "for continuous variables and factors or character vectors for",
               "discrete variables."))

  if ( length(X) != length(Y))
    stop(paste("The two samples must have the same number of observation",
               "(found", length(X), "and", length(Y), ")."))

  if ((!is.null(sample_weights)) && (length(sample_weights) != length(X)))
    stop(paste("The sample weight vector must be of the same length as the ",
               "number of observations (found", length(sample_weights),
               "while there are", length(X), "observations)."))

  if ((!is.null(matrixU) && !is.matrix(matrixU)) ||
      (!is.null(matrixU) && nrow(matrixU) != length(X)))
    stop(paste("matrixU is not a matrix or its number of rows differs from ",
               "the number of observations."))

  if ( !is.null(is_discrete) && ( length(is_discrete) != (2+nbrU)))
    stop(paste("The vector passed as is_discrete argument must be the same",
               "length as the number of variables, which is ncol(matrixU)+2."))
  ...
  if (length(which(NArows))>0) {
    warning(paste("Found", length(which(NArows)),
                  "NAs in at least one of the inputs. Running on",
  ...
  if (cplx == "mdl") {
      ...
      warning(paste("cplx parameter not understood, please specify either",
  ...
  if (base::requireNamespace("Rcpp", quietly = TRUE)) {
      rescpp <- .Call('mydiscretizeMutual', X, Y, flatU, nbrU, maxbins,
                      initbins, intcplx, cnt_vec, nlevels, Neff,
                      sample_weights, PACKAGE = "miic")
  }
...
# Take midpoints between two consecutive unique values instead of the values themselves
          clean_cutpoints = sapply(clean_cutpoints,,
                                   function(x) {
                                     if (x < uniquedata[length(uniquedata)])
                                       ((min(uniquedata[uniquedata > x]) +
                                         max(uniquedata[uniquedata <= x])) /
                                          2)
                                     else x
                                   })
        }
        ...
        if (max(clean_cutpoints) < uniquedata[length(uniquedata)])
          clean_cutpoints = c(clean_cutpoints,
  ...
  if (plot) {
    if (require(ggplot2) & require(gridExtra)) {
      if (!any(is_discrete[1:2])) {
        result$plot =
          jointplot_hist(X, Y, result, nameDist1, nameDist2)
      } else if (!all(is_discrete[1:2])) {
        result$plot =
          barplot_disc(X_orig, Y_orig, result, is_discrete, nameDist1, nameDist2)
      }
    } else {
...
jointplot_hist <- function(X, Y, result, nameDist1, nameDist2,
    ...
    bin_count = table(cut(X, cut_points1, include.lowest = T),
    ...
    fill_density_flat = data.frame(xstart = numeric(), xend = numeric(),
                                   ystart = numeric(), yend = numeric(),
                                   density = numeric())
    for(j in 1:(ncol(fill_density))) {
      for(i in 1:(nrow(fill_density))) {
        fill_density_flat[(j-1)*nrow(fill_density)+i, ] = c(cut_points1[i],
                                                            cut_points1[i + 1],
                                                            cut_points2[j],
                                                            cut_points2[j + 1],
                                                            fill_density[i, j])
    ...
    hist2d = ggplot(fill_density_flat) +
      geom_rect(aes(xmin = xstart, xmax = xend, ymin = ystart, ymax = yend,
                    fill = log(density)), na.rm = T, show.legend = F) +
      scale_fill_gradient(low = "#f4f5fc", high = "#0013a3",
                          position = "left", na.value = "white", limits =
                            log(c(min_density, max(fill_density_flat$density,
                                                   na.rm = T)))) +
      geom_vline(xintercept = cut_points1, linetype = "dashed",
                 color = "grey") +
      geom_hline(yintercept = cut_points2, linetype = "dashed",
                 color = "grey") +
      geom_point(data = data.frame(X, Y), aes(x = X, y = Y), shape = 21,
                 alpha = .7, fill = "#ffef77", size = 2) +
    ...
    geom_density(adjust = 0.5, alpha = .5, fill = "#c1c6ee") +
    theme_side_hist() %+replace% theme(plot.margin = margin(5.5, 5.5, -25,
                                                            5.5, "pt")) +
    scale_y_continuous(labels = labels, breaks = seq(0, 0.1,
                                                     length.out =
  ...
  return(gridExtra::grid.arrange(side_hist_top, empty, hist2d, side_hist_bot,
                                 ncol = 2, nrow = 2, widths = c(4.2, 1),
                                 heights = c(1, 4.2)))#, bottom=title))
}


barplot_disc = function(X, Y, result, is_discrete, nameDist1, nameDist2,
                        title = "Joint histogram") {
  ...
  barplot = ggplot(plot_df, aes(x = X, y = Y)) +
    ...
    geom_hline(yintercept = cut_points, linetype = "dashed", color = "grey",
               size = 1) +
    theme_classic() +
    xlab(Xname) +
    ylab(Yname)
