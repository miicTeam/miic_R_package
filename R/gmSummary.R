gmSummary <- function(inputData=NULL, edges = NULL, trueEdges = NULL,
  ...
  edgesNbr = 0
  edges.list = list(nOrt = list(idx = data.frame(nrow = numeric(0),
                                                 ncol = numeric(0)),
                                val = numeric(0), key = character(0)),
                    ort = list(idx = data.frame(nrow = numeric(0),
                                                ncol = numeric(0)),
                               val = numeric(0), key = character(0)),
                    ph = list(idx = data.frame(nrow = numeric(0),
                                               ncol = numeric(0)),
                              val = numeric(0), key = character(0)))
  ...
  outputSummary.df <- data.frame(x = character(edgesNbr),
  ...
  for (iEdgeCategory in names(edges.list)) {
    if (nrow(edges.list[[iEdgeCategory]][["idx"]]) > 0) {
      ...
      edges.list[[iEdgeCategory]][["idx"]] = t(apply(edges.list[[iEdgeCategory]][["idx"]],
                                                     MARGIN = c(1), FUN = function(x) {
                                                       x = gV$allProperties[x] 
                                                     })
                                               )

      #### Compute the key of each edge
      edges.list[[iEdgeCategory]][["key"]] = t(apply(edges.list[[iEdgeCategory]][["idx"]],
                                                     MARGIN = c(1), FUN = function(x) {
                                                       x = binVectToStr(x, gV)
                                                     }
                                                     )
  ...
  rownames(outputSummary.df) <- do.call(c, list(edges.list$nOrt$key,
                                                edges.list$ort$key,
                                                edges.list$ph$key))
  ...
  if (!is.null(true.edgesList)) {
    ...
    if (length(allPh.idx) > 0) {
      ...
      if (length(FN.type.idx) > 0)
        outputSummary.df[rownames(true.edgesList)[FN.type.idx], "type"] <- 'FN'
    }
    #### ----
    ...
    if (length(FN.type.idx) > 0) {
      ...
      FN.ort <- sapply(rownames(true.edgesList)[FN.type.idx]
                        , FUN=function(myKey) {
                          ifelse(isTRUE(all.equal(outputSummary.df[myKey,
                                                                   c("x", "y")],
    ...
    if (length(TP.type.idx) > 0) {
      ...
      TP.ort <- sapply(rownames(outputSummary.df)[TP.type.idx],
                       FUN=function(myKey) {
                         ...
                         ifelse((outputSummary.df[myKey, "x"] == true.edgesList[myKey, "x"] &
                                   outputSummary.df[myKey, "y"] == true.edgesList[myKey, "y"]), 2, -2)
                         }
                       )
  ...
  if ("cplx" %in% colnames(allMutInfo.df)) {
    ...
    outputSummary.df[, "log_confidence"] =
      as.numeric(
        as.numeric(outputSummary.df[, "info"]) -
  ...
  outputSummary.df <- outputSummary.df[order(as.numeric(
    outputSummary.df[, "log_confidence"]),
    decreasing = TRUE), ]
  ...
  if (!is.null(trueEdges)) {
    ...
    retListOrient = computeSHD2SampleStat(myTrueGraph = trueEdges,
  ...
  outputSummary.df <- outputSummary.df[, !colnames(outputSummary.df) %in%
                                         c("essential")]
  rownames(outputSummary.df) <- NULL
  res$all.edges.summary <- outputSummary.df


  outputSummary.df <- outputSummary.df[which(outputSummary.df$type %in%
                                              c("P", "TP", "FP")), ]
