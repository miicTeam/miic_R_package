gmSummary <- function(inputData = NULL,
                      edges = NULL,
                      trueEdges = NULL,
                      stateOrder = NULL,
                      adjMatrix = NULL,
  ...
  if (length(trueEdges) > 0) {
    ...
    rownames(true.edgesList) <- apply(true.edgesList, MARGIN = 1,
                                      function(myRow) {
                                        binVectToStr(myRow[1:2], gV)
                                      })
  }
  ...
  # -- not oriented
  edges.list[["nOrt"]][["idx"]] <-
    which(adjMat == 1, arr.ind = TRUE)
  edges.list[["nOrt"]][["val"]] <-
    rep(1, nrow(edges.list[["nOrt"]][["idx"]]))
  ...
  if (ortN > 0) {
    ...
    edges.list[["ort"]][["idx"]] <- data.frame(nrow = numeric(ortN),
                                               ncol = numeric(ortN))
    ...
    for (iOrt in c(2,-2, 4,-4, 6)) {
      ...
      if (tmp.ortN > 0) {
        edges.list[["ort"]][["idx"]][(countRow + 1):(countRow + tmp.ortN),] <-
          tmp.ortArr.idx
        edges.list[["ort"]][["val"]][(countRow + 1):(countRow + tmp.ortN)] <-
          tmp.ortArr.val
  ...
  if (nrow(edges.list[["ph"]][["idx"]]) > 0) {
    edges.list[["ph"]][["idx"]] <-
      edges.list[["ph"]][["idx"]][which(edges.list[["ph"]][["idx"]][, 2] > edges.list[["ph"]][["idx"]][, 1]), , drop =
                                    FALSE]
    edges.list[["ph"]][["val"]] <-
      rep(0, nrow(edges.list[["ph"]][["idx"]]))
  }
  edgesNbr = edgesNbr + length(edges.list[["ph"]][["val"]])

  #### Initialize the output data frame
  ### LV // adding 2 columns: signs & partial_correlation
  outputSummary.df <- data.frame(
    x = character(edgesNbr),
    y = character(edgesNbr),
    type = character(edgesNbr),
    ai = character(edgesNbr),
    info = numeric(edgesNbr),
    cplx = numeric(edgesNbr),
    Nxy_ai = numeric(edgesNbr),
    log_confidence = numeric(edgesNbr),
    infOrt = numeric(edgesNbr),
    trueOrt = numeric(edgesNbr),
    isOrt = character(edgesNbr),
    isOrtOk = character(edgesNbr),
  ...
  for (iEdgeCategory in names(edges.list)) {
    if (nrow(edges.list[[iEdgeCategory]][["idx"]]) > 0) {
      ...
      edges.list[[iEdgeCategory]][["idx"]] = t(apply(
        edges.list[[iEdgeCategory]][["idx"]],
        MARGIN = c(1),
        FUN = function(x) {
          x = gV$allProperties[x]
        }
      ))

      #### Compute the key of each edge
      edges.list[[iEdgeCategory]][["key"]] = t(apply(
        edges.list[[iEdgeCategory]][["idx"]],
        MARGIN = c(1),
        FUN = function(x) {
          x = binVectToStr(x, gV)
      ...
      outputSummary.df[c((countRow + 1):lastRow),] <- data.frame(
        ...
        ,
        isOrt = rep(NA, currentN),
        isOrtOk = rep(NA, currentN)
        ,
  ...
  rownames(outputSummary.df) <- do.call(c,
                                        list(edges.list$nOrt$key,
                                             edges.list$ort$key,
                                             edges.list$ph$key))
  ...
  if (!is.null(true.edgesList)) {
    ...
    if (ncol(true.edgesList) == 3)
      colnames(true.edgesList)[3] = "essential"
    ...
    if (length(allEdge.idx) > 0) {
      ...
      TP.type.idx <- which(abs(outputSummary.df[, "infOrt"]) > 0 &
                             (
                               rownames(outputSummary.df) %in%
                                 rownames(true.edgesList)
                             ))

      if (length(TP.type.idx) > 0)
        outputSummary.df[TP.type.idx, "type"] <- 'TP'
    }
    #### ----
    ...
    if (length(allPh.idx) > 0) {
      ...
      FN.type.idx <- which(rownames(true.edgesList) %in%
                             rownames(outputSummary.df)[allPh.idx])

      if (length(FN.type.idx) > 0)
        outputSummary.df[rownames(true.edgesList)[FN.type.idx], "type"] <-
        'FN'
    }
    #### ----
    ...
    if (length(FN.type.idx) > 0) {
      ...
      FN.ort <- sapply(
        rownames(true.edgesList)[FN.type.idx]
        ,
        FUN = function(myKey) {
          ifelse(isTRUE(all.equal(
            outputSummary.df[myKey,
                             c("x", "y")],
            true.edgesList[myKey,
                           c("x", "y")]
          )),
          2,-2)
        }
    ...
    if (length(TP.type.idx) > 0) {
      ...
      TP.ort <- sapply(
        rownames(outputSummary.df)[TP.type.idx],
        FUN = function(myKey) {
          # print("\n")
          ...
          ifelse((
            outputSummary.df[myKey, "x"] == true.edgesList[myKey, "x"] &
              outputSummary.df[myKey, "y"] == true.edgesList[myKey, "y"]
          ),
          2,
          -2
          )
    ...
    #### ----
    # -- oriented TP edge
    ortEdge.idx = which((abs(outputSummary.df[, "infOrt"]) > 1) &
                          (outputSummary.df[, "type"] == 'TP'))
    if (length(ortEdge.idx) > 0)
      outputSummary.df[ortEdge.idx, "isOrt"] = 'Y'
    ...
    if (length(compatibleOrt.idx) > 0)
      outputSummary.df[compatibleOrt.idx, "isOrtOk"] = 'Y'

    # -- not compatible orientation
    notCompatibleOrt.idx = which((outputSummary.df[, "type"] == 'TP') &
                                   (is.na(outputSummary.df[, "isOrtOk"])))
    if (length(notCompatibleOrt.idx) > 0)
      outputSummary.df[notCompatibleOrt.idx, "isOrtOk"] = 'N'
    #### ----

    #### Add the essentiality info if any
    if ("essential" %in% colnames(true.edgesList))
      outputSummary.df[rownames(true.edgesList), "essential"] <-
      true.edgesList[, "essential"]
  } else {
    ...
    remainingEdges.idx <-
      which(abs(outputSummary.df[, "infOrt"]) > 0)
    if (length(remainingEdges.idx) > 0)
      outputSummary.df[rownames(outputSummary.df[remainingEdges.idx,]), "type"] <-
      'P'
  }

  #### Use the mutual information values
  #### ----
  #### Add the mutual information values, the cplx values and the difference
  # (ie, confidence) if the cplx exisits...
  if ("cplx" %in% colnames(allMutInfo.df)) {
    outputSummary.df[rownames(outputSummary.df), c("info", "cplx")] <-
      allMutInfo.df[rownames(outputSummary.df), c("Ixy_ai", "cplx")]
    outputSummary.df[, "log_confidence"] =
      as.numeric(as.numeric(outputSummary.df[, "info"]) -
                   as.numeric(outputSummary.df[, "cplx"]))

  } else {
    outputSummary.df[rownames(outputSummary.df), c("info")] <-
      allMutInfo.df[rownames(outputSummary.df), c("Ixy_ai")]
  }

  #### Add the Nxy_ui if it exists
  if ("Nxy_ai" %in% colnames(allMutInfo.df))
    outputSummary.df[rownames(outputSummary.df), "Nxy_ai"] <-
    allMutInfo.df[rownames(outputSummary.df), "Nxy_ai"]

  #### Add the {ai}
  if ("ai.vect" %in% colnames(allMutInfo.df))
    outputSummary.df[rownames(outputSummary.df), "ai"] <-
    allMutInfo.df[rownames(outputSummary.df), "ai.vect"]

  #### Order by decreasing value of confidence
  outputSummary.df <- outputSummary.df[order(as.numeric(outputSummary.df[, "log_confidence"]),
                                             decreasing = TRUE),]


  ### AJOUT LV ###
  ### Compute the sign of each edge and fill the two last columns
  # (sign and partial correlation)
  if (verbose == TRUE)
  ...
  if (length(allNAs.idx) > 0)
    inputData.df = inputData.df[-allNAs.idx,]
  ...
  if (correlationToEvaluate)
    outputSummary.df[, c("sign", "partial_correlation")] <-
    computeSign.pcor(outputSummary.df, gV)

  res = list()

  if (!is.null(trueEdges)) {
    #### Compute complementary stats
    #### Open the final summary and count the number of TP, FP, TN, FN, TPnort,
    ...
    retListOrient = computeSHD2SampleStat(
      myTrueGraph = trueEdges,
      myFinalSummary = outputSummary.df,
      skeletonStats = retListSkeleton,
      myInfMethod = infMethod,
      myAllProperties = gV$allProperties
    )
  ...
  outputSummary.df <-
    outputSummary.df[,!colnames(outputSummary.df) %in%
                       c("essential")]
  ...
  outputSummary.df <-
    outputSummary.df[which(outputSummary.df$type %in%
                             c("P", "TP", "FP")),]
