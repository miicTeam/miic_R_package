...
computeSHD2SampleStat <- function(myTrueGraph, myFinalSummary, skeletonStats,
                                  myInfMethod, myAllProperties, myVb = FALSE,
                                  plotDir = NULL) {
	#### Scores to return	
	myErrWarKeys = c("infCpdagErrWar", "infVstructErrWar", "infSkelErrWar",
	                 "trueCpdagErrWar", "trueVstructErrWar", "trueSkelErrWar")
	myKeys = c("distance__pattern", "distance__cpdag", "distance__inferred",
	           myErrWarKeys)
	...
	graphToCompare = lapply(graphToCompare, function(x) {
	  tmpList  = vector("list", length(myKeys))
	  names(tmpList) = myKeys; x = tmpList })
  ...
  # the third column, if any, is for the essentiality
  trueGraph.edgelist = trueGraph.edgelist[, c(1, 2)]
  #### Define an empty true graph and add the edges from the list
	graphToCompare[["true"]][["graph"]] = bnlearn::empty.graph(myAllProperties)
	
	# Set the edges and their direction (we suppose here that all edges are
	# oriented as col1->col2)
	for(iTrueEdge in seq_len(nrow(trueGraph.edgelist))) {
        ...
        graphToCompare[["true"]][["graph"]] =
          bnlearn::set.arc(x = graphToCompare[["true"]][["graph"]], from = myX,
                           to= myY, check.cycles = FALSE, debug = myVb)
	}
	...
	myLinesToIgnore <- which(infEdges.summary[, "type"] == 'TN' |
	                         infEdges.summary[, "type"] == 'FN')
	if (length(myLinesToIgnore) > 0) {
	  infEdges.summary = infEdges.summary[-myLinesToIgnore, ]} 

	#### Define first an empty graph for the inferred one
	graphToCompare[["inferred"]][["graph"]] =
	  bnlearn::empty.graph(myAllProperties)
	
	# Set the edges and their direction if any
	for(iInfEdge in seq_len(nrow(infEdges.summary))) {
		#### Get the direction
		# - if '2', then x --> y, - if '-2', then y --> x, - if '1', set an
	  # undirected edge
		...
		if (abs(myDirection) > 0)
		{
			...
			{
				graphToCompare[["inferred"]][["graph"]] =
				  bnlearn::set.arc(x = graphToCompare[["inferred"]][["graph"]], from = myX,
				                   to= myY, check.cycles = FALSE, debug = myVb)
			} else if (myDirection == (-2))	{
				graphToCompare[["inferred"]][["graph"]] =
				  bnlearn::set.arc(x = graphToCompare[["inferred"]][["graph"]], from = myY,
				                   to= myX, check.cycles = FALSE, debug = myVb)
			} else if (myDirection == 1) {
			#} else if (myDirection %in% c(1,6)) {
				graphToCompare[["inferred"]][["graph"]] =
				  bnlearn::set.edge(x = graphToCompare[["inferred"]][["graph"]],
				                    from = myX, to= myY, check.cycles = FALSE,
				                    debug = myVb)
			}
		} 
	}
	...
	for(strType in names(graphToCompare)) {
		...
		myTry = tryCatch(bnlearn::cpdag(graphToCompare[[strType]][["graph"]],
		                                moral = TRUE,  debug = myVb), 
		...
		if (!(inherits(myTry, "simpleError") | inherits(myTry, "simpleWarning"))) {
			...
			myRetList[[myKey]] = gsub("\n", "", as.character(myTry))
			graphToCompare[[strType]][["cpdag"]] =
			  bnlearn::cpdag(graphToCompare[[strType]][["graph"]], moral = TRUE,
			                 debug = myVb)
		...
		if (!(inherits(myTry, "simpleError") | inherits(myTry, "simpleWarning"))) {
			...
			# if it is only a warning
			myRetList[[myKey]] = gsub("\n", "", as.character(myTry))
			graphToCompare[[strType]][["skeleton"]] =
			  bnlearn::skeleton(graphToCompare[[strType]][["graph"]])
			assign("last.warning", NULL, envir = baseenv())
		}

		# - the vStructures
		myKey = ifelse(strType == "true", "trueVstructErrWar", "infVstructErrWar")
		myTry = tryCatch(bnlearn::vstructs(graphToCompare[[strType]][["graph"]],
		                                   moral = FALSE, debug = myVb), 
											error = function(e) { e }, 	warning = function(w) { w })
		...
		if (!(inherits(myTry, "simpleError") | inherits(myTry, "simpleWarning"))) {
			...
			# if it is only a warning
			myRetList[[myKey]] = gsub("\n", "", as.character(myTry))
			graphToCompare[[strType]][["vstructs"]] =
			  bnlearn::vstructs(graphToCompare[[strType]][["graph"]], moral = FALSE,
			                    debug = myVb)
			assign("last.warning", NULL, envir = baseenv())
		}
		...
		{
			# - Go through the table of vstructures and set the orientation
		  # X --> Z and Y --> Z on the skeleton
			graphToCompare[[strType]][["pattern"]] =
			  graphToCompare[[strType]][["skeleton"]]
			if (!is.null(graphToCompare[[strType]][["vstructs"]])) {
				for(iVstruct in seq_len(nrow(graphToCompare[[strType]][["vstructs"]]))) {
					...
					for(nBase in c("X", "Y")) {
						...
						graphToCompare[[strType]][["pattern"]] =
						  bnlearn::set.arc(x = graphToCompare[[strType]][["pattern"]],
						                   from = strFrom, to= strTo, check.cycles = FALSE,
						                   debug = myVb)	
					}
				}
			}
	...
	if (!is.null(graphToCompare[["true"]][["cpdag"]]) &
	    !is.null(graphToCompare[["inferred"]][["graph"]])) {
		infEdges.summary.save =
		  computeSHD2(myTrueGraph = graphToCompare[["true"]][["cpdag"]],
		              myInfGraph = graphToCompare[["inferred"]][["graph"]],
		              infEdges.summary.save)
...
computeSHD2 <- function(myTrueGraph, myInfGraph, infEdges.summary.save,
                        myVb = FALSE) {

	#### Compute the distance between the two graph (shd2)
	#### ------------------------------------------------------------
	# - For each true oriented edge
	# -- if the edge exists in inferred and is oriented in opposite direction or not
  # oriented --> +1
	...
	for(iOriented in seq_len(nrow(bnlearn::directed.arcs(myTrueGraph)))) {
		...
		# if the edge has not been inferred (FN) or has been inferred with the true
		# direction --> OK
		# else shd2 +1
		if (!((bnlearn::amat(myInfGraph)[myEdge["from"], myEdge["to"]] == 0 &
		       bnlearn::amat(myInfGraph)[myEdge["to"], myEdge["from"]] == 0) |
		      (bnlearn::amat(myInfGraph)[myEdge["from"], myEdge["to"]] == 1 &
		       bnlearn::amat(myInfGraph)[myEdge["to"], myEdge["from"]] == 0))) {
			shd2 = (shd2 + 1)
			infEdges.summary.save[which((infEdges.summary.save[, "x"] == myEdge["from"] &
			                             infEdges.summary.save[, "y"] == myEdge["to"]) |
			                             (infEdges.summary.save[, "x"] == myEdge["to"] &
			                              infEdges.summary.save[, "y"] == myEdge["from"])),
			                      "isOrtOk"] = "N"
		} else {
			infEdges.summary.save[which(infEdges.summary.save[, "x"] == myEdge["from"] &
		                              infEdges.summary.save[, "y"] == myEdge["to"] |
	...
	for(iNotOriented in seq_len(nrow(bnlearn::undirected.arcs(myTrueGraph)))) {
		...
		if (length(myIdenticalEdge) == 1) {
		  isDone[myIdenticalEdge] = 1
		} else {
	    stop("# Only one non Oriented (",
	         paste(myEdge, collapse = "--"),
	         ") in the table!\n", sep = "")
	  }
		...
		if (!((bnlearn::amat(myInfGraph)[myEdge["from"], myEdge["to"]] == 0 &
		       bnlearn::amat(myInfGraph)[myEdge["to"], myEdge["from"]] == 0) |
		      (bnlearn::amat(myInfGraph)[myEdge["from"], myEdge["to"]] == 1 &
		       bnlearn::amat(myInfGraph)[myEdge["to"], myEdge["from"]] == 1))) {
			shd2 = (shd2 + 1)
			infEdges.summary.save[which(infEdges.summary.save[, "x"] == myEdge["from"] &
			                            infEdges.summary.save[, "y"] == myEdge["to"] |
		                              (infEdges.summary.save[, "x"] == myEdge["to"] &
    ...
    res$stats = matrix(NA, ncol	= 3 , nrow = 2,
                       dimnames = list(c("Skeleton", "CPDAG"),
