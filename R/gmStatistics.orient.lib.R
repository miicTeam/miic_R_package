...
computeSHD2SampleStat <-
  function(myTrueGraph,
           myFinalSummary,
           skeletonStats,
           myInfMethod,
           myAllProperties,
    ...
    myKeys = c("distance__pattern",
               "distance__cpdag",
               "distance__inferred",
               myErrWarKeys)
    myRetList = vector("list", length(myKeys))
    names(myRetList) = myKeys
    myRetList = lapply(myRetList, function(x)
      x = NA)
    ...
    for (iInfEdge in seq_len(nrow(infEdges.summary))) {
      ...
      if (abs(myDirection) > 0)
      {
        ...
        if (myDirection == 2)
        {
          graphToCompare[["inferred"]][["graph"]] =
            bnlearn::set.arc(
              x = graphToCompare[["inferred"]][["graph"]],
              from = myX,
              to = myY,
              check.cycles = FALSE,
              debug = myVb
    ...
    for (strType in names(graphToCompare)) {
      ...
      myTry = tryCatch(
        bnlearn::cpdag(graphToCompare[[strType]][["graph"]],
      ...
      if (!(inherits(myTry, "simpleError") |
            inherits(myTry, "simpleWarning"))) {
      ...
      myTry = tryCatch(
        bnlearn::vstructs(graphToCompare[[strType]][["graph"]],
        ...
        if (!is.null(graphToCompare[[strType]][["vstructs"]])) {
          for (iVstruct in seq_len(nrow(graphToCompare[[strType]][["vstructs"]]))) {
            ...
            for (nBase in c("X", "Y")) {
              ...
              graphToCompare[[strType]][["pattern"]] =
                bnlearn::set.arc(
                  x = graphToCompare[[strType]][["pattern"]],
                  from = strFrom,
                  to = strTo,
                  check.cycles = FALSE,
                  debug = myVb
...
computeSHD2 <-
  function(myTrueGraph,
    ...
    for (iOriented in seq_len(nrow(bnlearn::directed.arcs(myTrueGraph)))) {
      ...
      # else shd2 +1
      if (!((
        bnlearn::amat(myInfGraph)[myEdge["from"], myEdge["to"]] == 0 &
      ...
      (
        ...
        shd2 = (shd2 + 1)
        infEdges.summary.save[which((
          infEdges.summary.save[, "x"] == myEdge["from"] &
    ...
    for (iNotOriented in seq_len(nrow(bnlearn::undirected.arcs(myTrueGraph)))) {
      ...
      myIdenticalEdge = which(
        bnlearn::undirected.arcs(myTrueGraph)[, "from"] == myEdge["to"] &
