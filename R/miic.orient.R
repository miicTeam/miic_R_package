miic.orient <-
  function(inputData = NULL,
    ...
    if (length(res$orientations.prob) > 0) {
      tmp <-
        unlist(res$orientations.prob)[1:length(res$orientations.prob[[1]])]
      res1 <-
        unlist(res$orientations.prob)[(length(res$orientations.prob[[1]]) + 1):length(unlist(res$orientations.prob))]
      df <-
        data.frame(matrix(
          res1,
          nrow = length(res$orientations.prob) - 1,
          byrow = T
        ),
    ...
    res1 <-
      unlist(res$adjMatrix)[(length(res$adjMatrix[[1]]) + 1):length(unlist(res$adjMatrix))]
    df <-
      data.frame(matrix(res1, nrow = length(res$adjMatrix) - 1, byrow = T),
                 stringsAsFactors = FALSE)
    df <- df[, -1]
    colnames(df) <- tmp
