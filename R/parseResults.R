...
summarizeResults <- function(observations=NULL, edges = NULL, true_edges = NULL,
                         ...
                         verbose = FALSE)
{
    ...
    if(is.null(true_edges)){
        summary$type = apply(summary, 1, function(row, adj_matrix) {
            ifelse(adj_matrix[row[1], row[2]]==0, 'N', 'P')}, adj_matrix)
    } else{
        type = character(n)
        for(i in 1:nrow(summary)){
            row = summary[i,]
            row_is_true_edge = any(apply(true_edges, 1,
                                   function(true_edge, edge){all(true_edge %in% edge)},
                                   c(row$x, row$y)))
    ...
    if(is.null(true_edges)) {
        ...
        true_adj_matrix = matrix(0, ncol=dim(adj_matrix)[1],
...
matrix_from_3_columns_igraph = function(matrix, rows, columns, values){
    ...
    g <- igraph::graph.data.frame(matrix[,c(rows,columns,values)], directed=FALSE)
...
matrix_from_3_columns = function(matrix, rows, columns, values){
    ...
    colnames(wide_matrix) <- substr(colnames(wide_matrix), nchar(values)+2,
...
fill_summary_column = function(summary, matrix, rows, columns, values){
    ...
    apply(summary, 1, function(row, wide_matrix){
        wide_matrix[row[1], row[2]]}, wide_matrix)
}
...
compute_partial_correlation = function(summary, observations, state_order){
    ...
    ppcor_results = data.frame(sign=character(nrow(summary)),
                               partial_correlation=numeric(nrow(summary)),
                               stringsAsFactors=FALSE)

    if(!is.null(state_order) && 'levels_increasing_order' %in% colnames(state_order)){
    ...
    for(i in 1:nrow(summary)){
        ...
        non_na_rows = apply(observations, 1, function(row, columns){
            !any(is.na(row[columns]))}, c(x,y,ai))

        if(!is.null(ai)){
            ai = unlist(strsplit(ai, ',')) # Character vector

            edge_res = ppcor::pcor.test(observations[,x],  observations[,y],
                                        observations[,ai], method="spearman")
        } else{
            edge_res = cor.test(observations[,x], observations[,y], method="spearman")
        }

        # Save sign and coef
        ppcor_results[i,] = c(ifelse(edge_res$estimate>=0, '+', '-'),
                              edge_res$estimate)
...
is_causal = function(summary, probas) {
    ...
    if(length(which(v_structs)) == 0) return(is_causal_results)

    vstruct_matrix = matrix_from_3_columns(probas[v_structs,], "source1",
                                           "source2", "target")
    error_matrix   = matrix_from_3_columns(probas[v_structs,], "source1",
                                           "source2", "Error")
